        ...
        {loading ? (
          ...
          <div className="space-y-1 border-t border-white/10 pt-4">
            {results.filter(item => {
              ...
              if (item.type === 'ARTIST') return <ArtistItem key={`artist-${item.artistId || index}`} artist={item} />;
              if (item.type === 'ALBUM') return <AlbumItem key={`album-${item.browseId || item.albumId || index}`} album={item} />;
              if (item.type === 'PLAYLIST') return <PlaylistItem key={`playlist-${item.browseId || item.playlistId || index}`} playlist={item} />;
              return <TrackItem key={`track-${item.videoId || index}`} track={item} queue={results.filter(r => r.type === 'SONG' || r.type === 'VIDEO')} />;
            })}
          </div>
        ) : query ? (
